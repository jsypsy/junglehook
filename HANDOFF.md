# Session Handoff
_Last updated: 2026-08-30 (BUILD 20)_

## 지금 상태

**정글훅 — 카툰 정글 + 사계절 + 이어하기 광고 루프 + 슈퍼 모드 150m + 뒤쫓는 폭풍 + 일수 표기 (BUILD 20). 폭풍 실기기 판정 대기.**

```
컨셉       원버튼 그래플 스윙 (홀드=로프+감기+펌프, 릴리스=관성 발사). D-001, D-005, D-006
이름       정글훅 / junglehook — 충돌 검색 완료. GRAC 제명은 신청 직전 재확인 (D-002)
코드       BUILD 20 — **일수 표기(D-014: 1000m=365일, 표시만)** + **뒤쫓는 폭풍(D-013: 300→420px/s, 고무줄 1000, 잡히면 즉사 · `?p=nothreat`/`chase2`)** +
...
다음       **폭풍 실기기 판정(B20 배포됨)**(30~60초 세션? 잡힘이 내 탓으로 느껴지나?) → Phase 1 잔여(효과음·점수 제출·뒤로가기 모달)
```

## 다음 세션이 할 일

0. **폭풍 실기기 판정** (D-013, BUILD 20 배포·문자 전송 완료). 봇 계측으론 안전 봇 평균 45s에 잡힘 — 실제 손으로 볼 것:
