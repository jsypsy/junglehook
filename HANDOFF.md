...
코드       BUILD 16 — 소닉 파워(D-012, 계절당 1회 찬스 잎→3바퀴→게이지→250m 로켓 대시, 따뜻한 슈퍼 모드) + 캐노피 배경(D-011, 가을 낙엽·계절 태양 크기) + 사계절(D-010, 250m·oklab·벚꽃/눈·계절 태양) + 잎 앵커 +
           이어하기 광고 루프(D-009) + 카툰 렌더러(D-008) + 충전기 물리·`?p=b4` (D-006/7). 테스트 37개
실플레이   BUILD 1: 156m→206m(≈15s). 피드백은 전부 D-006/7에 반영됨. BUILD 8은 미판정
콘솔       junglehook 등록됨 · 테스트 배포 10회 (최신 BUILD 16) — tools/latest-deployment.json
...
2. 사계절·캐노피(BUILD 11) 실기기 확인: 250m 경계 보간, 겨울 눈 가독성, 캐노피 층 성능(맥 0.67ms/프레임).
   **소닉 파워(BUILD 16) 손맛 판정**: 찬스 잎이 눈에 띄는지, 3바퀴(≈7.5초)·게이지 난이도, 250m 대시 — `tuning.sonic`
