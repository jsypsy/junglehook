...
**정글훅 — 카툰 정글 + 사계절 + 이어하기 광고 루프 + 슈퍼 모드 150m + 뒤쫓는 폭풍 + 일수 표기 (BUILD 25). 계절 나무·벽괴물·랜덤 찬스·슈퍼 도전 대기 실기기 판정 진행 중.**

```
컨셉       원버튼 그래플 스윙 (홀드=로프+감기+펌프, 릴리스=관성 발사). D-001, D-005, D-006
이름       정글훅 / junglehook — 충돌 검색 완료. GRAC 제명은 신청 직전 재확인 (D-002)
코드       BUILD 25 — **계절 나무(가지 뼈대+잎 밀도, 스프라이트 캐시 · render/tree.ts)** + **슈퍼 도전 대기+매뉴얼 카드+게이지 위치 랜덤(D-016)** + **찬스 랜덤 발동(D-015: 계절마다 k번째 잡기, 게이지 0.06)** + **슈퍼 대시 200m(73일)** + **일수 표기(D-014: 1000m=365일, 표시만)** + **뒤쫓는 폭풍(D-013: 300→420px/s, 고무줄 1000, 잡히면 즉사 · 찬스 잎에선 물러나 정지 · `?p=nothreat`/`chase2`)** +
...
다음       **B25 실기기 판정** — 계절 나무 성능(폰 프레임)·겨울 앙상함/여름 풍성함 체감 · B24 항목(도전 대기·더블탭·게이지 랜덤)(30~60초 세션? 잡힘이 내 탓으로 느껴지나?) → Phase 1 잔여(효과음·점수 제출·뒤로가기 모달)
```

## 다음 세션이 할 일

0. **B25 실기기 판정** (배포·문자 전송 완료). B25: 계절 나무 렌더러 반영 + 태양 후광 여름만. 폰에서 프레임이 떨어지면
   `drawTreeLayer` 먼 나무 4→3 그루 또는 스프라이트 해상도(tree.ts scale)를 내린다. 이전 B24: B24: 찬스 → 멈춘 채 대기 → 떼었다 다시 누르면 도전(D-016), 매뉴얼 카드 2회 +
