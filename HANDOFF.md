# Session Handoff
_Last updated: 2026-08-30 (BUILD 22)_

## 지금 상태

**정글훅 — 카툰 정글 + 사계절 + 이어하기 광고 루프 + 슈퍼 모드 150m + 뒤쫓는 폭풍 + 일수 표기 (BUILD 22). 벽괴물 실기기 판정 진행 중.**

```
컨셉       원버튼 그래플 스윙 (홀드=로프+감기+펌프, 릴리스=관성 발사). D-001, D-005, D-006
이름       정글훅 / junglehook — 충돌 검색 완료. GRAC 제명은 신청 직전 재확인 (D-002)
코드       BUILD 22 — **슈퍼 대시 200m(73일)** + **일수 표기(D-014: 1000m=365일, 표시만)** + **뒤쫓는 폭풍(D-013: 300→420px/s, 고무줄 1000, 잡히면 즉사 · 찬스 잎에선 물러나 정지 · `?p=nothreat`/`chase2`)** +
...
다음       **벽괴물 실기기 판정 계속(B22: 대시 200m)** → 찬스 잡기 개선 여부 결정(자석·다음 잎)(30~60초 세션? 잡힘이 내 탓으로 느껴지나?) → Phase 1 잔여(효과음·점수 제출·뒤로가기 모달)
```

## 다음 세션이 할 일

0. **벽괴물 실기기 판정** (D-013 — 문서의 "폭풍"은 벽괴물과 같은 것). 피드백 반영 이력: 슈퍼 회전 중 잡힘 → B21 정지 ·
   278일(762m)에 "많이 어려워졌다" → B22 대시 200m. **미결정**: 찬스 잡기 개선(찬스 잎이 사거리 안이면 타깃 우선 "자석" +
   놓치면 다음 잎으로 후광 이동) — 지금은 selectTarget이 찬스를 안 보고 선호점 최근접만 골라 운에 좌우된다.
   난이도 노브 내릴 순서: maxLeadPx 1000→1200 · speedMin 300→270 · speedPerCycle 40→25. 봇 계측으론 안전 봇 평균 45s에 잡힘 — 실제 손으로 볼 것:
