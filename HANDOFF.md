...
**정글훅 — 카툰 정글 + 사계절 + 이어하기 광고 루프 + 슈퍼 모드 (BUILD 17). 실기기 판정 대기.**

```
컨셉       원버튼 그래플 스윙 (홀드=로프+감기+펌프, 릴리스=관성 발사). D-001, D-005, D-006
이름       정글훅 / junglehook — 충돌 검색 완료. GRAC 제명은 신청 직전 재확인 (D-002)
코드       BUILD 17 — 슈퍼 모드(D-012: 계절당 1회 찬스 잎 → "SUPER!" 배지 → 3바퀴 → 타이밍 게이지 →
           250m 로켓 대시, 백열·번개·블러) + 캐노피 정글 배경(D-011) + 사계절 250m(D-010) + 잎 앵커 +
           이어하기 광고 루프(D-009) + 카툰 렌더러(D-008) + 충전기 물리·`?p=b4`(D-006/7). 테스트 39개
...
다음       B17 실기기 판정 → tuning.sonic 조정 → Phase 1 잔여(효과음·점수 제출·뒤로가기 모달)
```
