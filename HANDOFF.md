# Session Handoff
_Last updated: 2026-08-30 (BUILD 23)_

## 지금 상태

**정글훅 — 카툰 정글 + 사계절 + 이어하기 광고 루프 + 슈퍼 모드 150m + 뒤쫓는 폭풍 + 일수 표기 (BUILD 23). 벽괴물·랜덤 찬스 실기기 판정 진행 중.**

```
컨셉       원버튼 그래플 스윙 (홀드=로프+감기+펌프, 릴리스=관성 발사). D-001, D-005, D-006
이름       정글훅 / junglehook — 충돌 검색 완료. GRAC 제명은 신청 직전 재확인 (D-002)
코드       BUILD 23 — **찬스 랜덤 발동(D-015: 계절마다 k번째 잡기, 게이지 0.06)** + **슈퍼 대시 200m(73일)** + **일수 표기(D-014: 1000m=365일, 표시만)** + **뒤쫓는 폭풍(D-013: 300→420px/s, 고무줄 1000, 잡히면 즉사 · 찬스 잎에선 물러나 정지 · `?p=nothreat`/`chase2`)** +
...
다음       **B23 실기기 판정** — 찬스가 계절마다 손에 들어오는가 · 게이지 86ms 성공률 · 세션 길이/난이도 체감(30~60초 세션? 잡힘이 내 탓으로 느껴지나?) → Phase 1 잔여(효과음·점수 제출·뒤로가기 모달)
```

## 다음 세션이 할 일

0. **B23 실기기 판정** (배포·문자 전송 완료). 이력: B19 벽괴물 → B21 슈퍼 회전 중 잡힘 수정 → 278일(762m) "많이
