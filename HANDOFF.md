# Session Handoff
_Last updated: 2026-08-30 (BUILD 18)_

## 지금 상태

**정글훅 — 카툰 정글 + 사계절(표정 포함) + 이어하기 광고 루프 + 슈퍼 모드 150m (BUILD 18). 실기기 판정·난이도 결정 대기.**

```
컨셉       원버튼 그래플 스윙 (홀드=로프+감기+펌프, 릴리스=관성 발사). D-001, D-005, D-006
이름       정글훅 / junglehook — 충돌 검색 완료. GRAC 제명은 신청 직전 재확인 (D-002)
코드       BUILD 18 — 슈퍼 모드(D-012: 계절당 1회 찬스 잎 → "SUPER!" 배지 → 3바퀴 → 타이밍 게이지 →
...
다음       **난이도 결정**(사용자 "너무 쉽다" — `?p=hard` A/B) → B18 실기기 판정 → Phase 1 잔여(효과음·점수 제출·뒤로가기 모달)
```

## 다음 세션이 할 일

0. **난이도 판정** — 사용자 "너무 쉽다". BUILD 18: 슈퍼 대시 150m + **2차 램프**(사계절 한 바퀴마다 간격 +45·흔들림 +25,
   상한 390/175 — `tuning.anchor.gapPerCycle/jitterPerCycle/gapCap/jitterCap`). 실기기에서 1000m·2000m 부근 손맛과
   "죽음이 내 탓으로 느껴지는가"를 판정. 더 필요하면 ① `?p=hard`(1차 램프 앞당김) ② 슈퍼 게이지 sweetHalf 0.08→0.06
   ③ reach 450→400(D-007 억울함과 트레이드오프). 계절별 규칙(여름 바람 등)은 계속 보류
