...
export const TUNING = {
  ...
  maxContinues: 2,
  /** 이어하기 재출발: 죽은 자리 x(단 다음 잎보다 dx 이상 앞), 다음 잎 dy 아래, 초기 속도는 정상 릴리스(+30°, 전방 위)와
   *  같은 궤적. 첫 잎(150 앞·130 위)과 같은 기하 — 마지막 잎 기준(+75,+130)은 다음 잎이 아래에 놓일 수 있었다 (BUILD 32) */
  continueSpawn: { dx: 150, dy: 130, vx: 520, vy: -300 },
  ...
  sonic: {
    ...
    chanceStepM: 1000,
    /** 찬스 발동 (D-015 → D-018): chanceStepM(1년)마다 한 번, **그 해의 어느 지점**에서 터진다.
     *  지점은 시드+단계로 정해지는 구간 비율 랜덤 [chanceFrom, chanceTo] — 그 지점을 지나 **내가 처음 잡는
     *  앵커**가 찬스 앵커가 된다. 잡는 앵커 어디서든 터지므로 "잡히느냐가 운"이던 옛 금색 잎(B15~22) 문제가 없고,
     *  잡기 횟수(k번째, B23~27)와 달리 사람마다 다른 잡기 빈도에 발동 위치가 흔들리지 않는다.
     *  같은 앵커를 다시 잡는 낚시도 무의미하다(지점을 지나면 다음 잡기에서 무조건) */
    chanceFrom: 0.05,
    chanceTo: 0.85,
    /** 첫 해만 앞쪽으로 — 슈퍼는 이 게임의 핵심 연출인데 1년 1회(D-017)라, 첫 판에서 850m까지 한 번도 못 보면
     *  매뉴얼 카드조차 뜨지 않는다. 첫 해는 [chanceFrom, firstChanceTo]. 0.1~0.3(100~300m)은 초보 봇(45°, 20~27초
     *  사망)의 절반이 찬스 전에 죽었다 → 0.05~0.2(50~200m): 17/24가 본다. 평균 플레이어 세션은 그대로 (BUILD 32) */
    firstChanceTo: 0.2,
